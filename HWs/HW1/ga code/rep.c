...
void rawStat(FILE *fp, POPULATION *p, IPTR pop)
{
  ...
  // fprintf(fp," gen : %3d max = %10.2lf %10.2lf %10.2lf ", p->gen, p->max,
	//  p->avg, p->min);
	
  fprintf(fp," gen : %3d max = %10.2lf ", p->gen, p->max);
  
  fprintf(fp," hG : %3d hF : %10.2lf hIDX : %3d  hCHR : ", p->highestEverGen,  p->highestEverFitness,
	  p->highestEverIndex);
	  
  for(int i = 0;i < p->lchrom;i++)
	fprintf(fp," %d", (p->max_fit).chrom[i]);
  fprintf(fp,"\n");
}
